...
import { field } from './ui/ui-helpers2.js';
...
import { initVendorPanel } from './ui/vendor-panel.js';
import { exportDebugLog } from '../debug.js';
import type { Group, GroupEntry, LogicalModel, MappingRule, RoutingSettings, Vendor } from '../types.js';
...
export function initRoutingUI(deps: RoutingUIDeps): { panel: JQuery<HTMLElement>; render(): void } {
    ...
    const vendorPanel = initVendorPanel({
        deps,
        mount: providerList,
        getActiveGroup: () => activeGroup(),
        reloadModelList: () => renderModelList(),
        reloadGroupSummary: () => renderGroupSummary(),
    });
    ...
    panel.find('#st_router_refresh_models').on('click', async () => {
        ...
        try {
            const { ok, skipped, failed } = await vendorPanel.batchFetchAllModels();
            vendorPanel.renderProviders();
            renderModelList();
            renderGroupSummary();
            const parts = [`成功 ${ok} 个`];
    ...
    return { panel, render: () => { renderRoutingControls(); renderGroupSelect(); vendorPanel.renderProviders(); renderModelList(); } };
}
