...
import type { GroupEntry, ModelFailureKind } from '../types.js';
...
/** 从 toastr 错误消息推断错误分类（纯被动，无 HTTP 返回码）。 */
export function classifyModelFailureMessage(msg: string): ModelFailureKind {
    const m = String(msg ?? '').toLowerCase();
    // 不可恢复：模型不存在 / 余额不足 / 无权限 / key 无效 / 封禁
    if (/(model[_ -]?not[_ -]?found|no such model|model .* not exist|insufficient[_ -]?quota|no quota|quota exhausted|balance|401|403|permission|banned|invalid api key|account disabled|access denied|forbidden|unauthorized|payment required)/i.test(m)) {
        return 'fatal';
    }
    // 限流
    if (/(429|rate ?limit|too many requests|rate limit exceeded|quota exceeded)/i.test(m)) {
        return 'rate_limited';
    }
    // 参数错误：不是渠道故障，不处理
    if (/(400|bad request|parameter|invalid request|invalid parameter|format|invalid input|validation error)/i.test(m)) {
        return 'bad_request';
    }
    // 网络类错误 / 超时 / 服务端错误 → temp
    if (/(failed to fetch|load failed|network|timed out|abort|timeout|5\d{2}|server error|service unavailable|internal error)/i.test(m)) {
        return 'temp';
    }
    return 'unknown';
}
